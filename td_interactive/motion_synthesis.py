...
class OneEuroFilterTorch:
    ...
    def __call__(self, x):
        ...
        # Estimate cutoff frequency per element based on velocity
...
class MotionSynthesis():
    ...
    def setGaussianTemp(self, gaussian_temp):
        ...
        print("self.gaussian_temp ", self.gaussian_temp)

    def setPiTemp(self, pi_temperature):
        self.pi_temperature = pi_temperature
        print("self.pi_temperature ", self.pi_temperature)

    def setTopP(self, top_p):
        self.top_p = top_p
        print("self.top_p ", self.top_p)

    def _orthogonalize_pose(self, pose):
        """
        Extracts 6D rotations from the pose tensor, applies Gram-Schmidt orthogonalization, 
        and recombines them with the root trajectory. Agnostic to batch/sequence dimensions.
        """
        ...
        # Reshape to isolate the joint count and 6D rotation components
        rot_6d = rot_6d.view(*pose_shape[:-1], self.joint_count, 6)

        # Use rot_to to convert to matrix and back to enforce orthogonality
    ...
    def setJointRotation(self, joint_index, joint_rot, frame_count):
        # Convert incoming quaternion to 6D format using Torch utilities
        joint_rot = torch.from_numpy(joint_rot).to(torch.float32).to(self.device)
        joint_rot_6d = rot_to.quat_to_r6d(joint_rot.unsqueeze(0)).squeeze(0)

        # Calculate the feature slice indices for the target joint
        start_idx = 3 + (joint_index * 6) if self.train_root_trajectory else joint_index * 6
        end_idx = start_idx + 6

        if frame_count == 1:
            self.motion_seq[-2, start_idx:end_idx] = joint_rot_6d
        elif frame_count > 1:
            frame_count = min(frame_count, self.seq_length)
            joint_rot_6d_expanded = torch.unsqueeze(joint_rot_6d, dim=0).repeat(frame_count, 1)
            self.motion_seq[:frame_count, start_idx:end_idx] = joint_rot_6d_expanded

    def changeJointRotation(self, joint_index, joint_rot, frame_count):
        joint_rot = torch.from_numpy(joint_rot).to(torch.float32).to(self.device)
        joint_rot = rot_to.normalize(joint_rot)
        ...
        if frame_count == 1:
            # Extract current 6D rotation and convert it back to a quaternion
            current_6d = self.motion_seq[-1, start_idx:end_idx]
            current_quat = rot_to.r6d_to_quat(current_6d)

            # Apply quaternion rotation
            new_quat = rot_to.mul(current_quat.unsqueeze(0), joint_rot.unsqueeze(0)).squeeze(0)

            # Convert back to 6D and assign
            new_6d = rot_to.quat_to_r6d(new_quat)
            self.motion_seq[-2, start_idx:end_idx] = new_6d

        elif frame_count > 1:
            frame_count = min(frame_count, self.seq_length)

            # Extract historical 6D sequence and convert to quaternions
            current_6d = self.motion_seq[:frame_count, start_idx:end_idx]
            current_quat = rot_to.r6d_to_quat(current_6d)

            joint_rot_expanded = torch.unsqueeze(joint_rot, dim=0).repeat(frame_count, 1)

            # Apply quaternion rotation
            new_quat = rot_to.mul(current_quat, joint_rot_expanded)

            # Convert sequence back to 6D and assign
            new_6d = rot_to.quat_to_r6d(new_quat)
            self.motion_seq[:frame_count, start_idx:end_idx] = new_6d
    ...
    def update(self):
        ...
        with torch.no_grad():
            log_pi, mu, sigma = self.model(torch.unsqueeze(self.motion_seq, axis=0), return_sequence=False)
            ...
            self.pred_pose = pred_pose_norm.reshape(1, self.input_dim)

        # Apply 1 Euro Filter and enforce orthogonality in a single line
        filtered_pose = self.one_euro_filter(self.pred_pose)
        ...
        pred_numpy = self.pred_pose.detach().cpu().numpy()
        ...
        # Calculate world positions and rotations using PyTorch
        ...
        # Process local rotations for OSC using NumPy utils
    ...
    def _sample_mdn(self, log_pi, mu, sigma, gaussian_temp=0.1, pi_temperature=1.5, top_p=0.9):
        ...
        # Enforce orthogonality before returning
