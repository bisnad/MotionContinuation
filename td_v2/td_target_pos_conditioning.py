...
def create_pred_sequence(pose_sequence, pose_count, initial_target_location, autonomous=True, interactive_target_stream=None, bounding_box=None):
    ...
    if bounding_box is not None:
        ...
        root_mean_t = torch.tensor(root_pos_mean.flatten(), device=device, dtype=torch.float32).view(1, 1, 3)
        root_std_t = torch.tensor(root_pos_std.flatten(), device=device, dtype=torch.float32).view(1, 1, 3)
