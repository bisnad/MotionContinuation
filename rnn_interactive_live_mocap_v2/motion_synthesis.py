...
class MotionSynthesis():
    ...
    def __init__(self, config):
        ...
        self.model_rnn = config["model_rnn"]
        ...
        self.live_seq = torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=torch.float32).repeat(self.seq_length, self.joint_count)
        self.live_seq = self.live_seq.reshape(self.seq_length, self.joint_count, 4)
        self.motion_seq = self.motion_seq.reshape(self.seq_length, self.joint_count, 4)
        self.live_pose = None
        self.live_seq_changed = False
        self.live_seq_ready = False
        self.live_seq_update_counter = 0
        self.live_seq_interpol_counter = 0
        self.live_seq_interpol_count = 100
    ...
    def updateLiveSeq(self, rotLocal):
        
        #if self.live_seq_ready == True:
        #    return
        ...
        self.live_seq = torch.cat([self.live_seq[1:, ...], self.live_pose.reshape(1, self.joint_count, 4)], axis=0)

        self.live_seq_changed = True
        self.live_seq_update_counter += 1
        
        if self.live_seq_update_counter == self.seq_length:
            self.live_seq_ready = True
        
    def initLiveSeq(self):
    ...
    def copyLiveSequence(self):
        
        """
        print("copyLiveSequence")
        
        #self.live_seq = torch.cat([self.live_seq[1:, ...], self.live_pose.reshape(1, self.joint_count, 4)], axis=0)
        self.motion_seq = self.live_seq.to(self.device)
        
        self.live_seq_changed = False
        """
        
        self.motion_seq = torch.cat([self.motion_seq[:-1, ...], self.live_seq[-1:, ...].to(self.device)], axis=0)
    ...
    def update(self):
        ...
        live_pose = self.live_pose.to(self.device)
        live_pose = live_pose.reshape(1, self.joint_count, self.joint_dim)
        ...
        with torch.no_grad():
            ...
            #print("motion_seq s ", self.motion_seq.shape)
            
            motion_seq = self.motion_seq.reshape(1, -1, self.pose_dim)
            motion_seq_enc_mu, motion_seq_enc_std  = self.model_encoder(motion_seq)
            motion_seq_enc_mu = torch.tanh(motion_seq_enc_mu)
            
            """
            motion_seq_enc_std = torch.abs(torch.tanh(motion_seq_enc_std)) + 0.00001
            motion_seq_enc = self._reparameterize(motion_seq_enc_mu, motion_seq_enc_std)
            """
            
            motion_seq_enc = motion_seq_enc_mu
            
            #print("motion_seq_enc s ", motion_seq_enc.shape)
            
            motion_seq_dec = self.model_decoder(motion_seq_enc)
            
            #print("motion_seq_dec s ", motion_seq_dec.shape)
            
            #self.pred_pose = self.model_rnn(motion_seq_dec)
            self.pred_pose = self.model_rnn(motion_seq)
            
            #print("pred_pose s ", self.pred_pose.shape)
        ...
        if self.live_seq_ready == False: 
            # append live pose to motion seq
            self.motion_seq = torch.cat([self.motion_seq[:-1, ...], live_pose], axis=0)
        
        else:
            # append pred pose to motion seq
            self.motion_seq = torch.cat([self.motion_seq[1:,:], self.pred_pose], axis=0)

        # blend between live pose and pred pose based on live_seq_interpol_counter
        # debug version
        ...
        if self.live_seq_ready == False: 
            ...
            blend_pose = self.pred_pose 
        """
        
        """
        # torch slerp really not working for some reason....
        blend_factor = torch.ones([self.joint_count], dtype=torch.float32).to(self.device) * self.orig_seq_blend_factor
        blend_pose = slerp(live_pose[0, ...], self.pred_pose[0, ...], blend_factor)
        blend_pose = blend_pose.reshape(1, self.joint_count, self.joint_dim)
        """
        ...
        if self.live_seq_ready == False: 
            blend_factor = 0.0 
        elif self.live_seq_interpol_counter < self.live_seq_interpol_count:
            blend_factor = self.live_seq_interpol_counter / self.live_seq_interpol_count
            self.live_seq_interpol_counter += 1
        else: blend_factor = 1.0
        
        #print("blend_factor ", blend_factor)

        # numpy slerp
        live_pose_np = live_pose.detach().cpu().numpy()
        ...
        self.synth_pose_lrot = blend_pose.detach().cpu().numpy()
        ...
        self.synth_pose_wpos, self.synth_pose_wrot = self._forward_kinematics(torch.unsqueeze(blend_pose,dim=0), zero_trajectory)
