...
config = {
    ...
    "osc_ip": "127.0.0.1",
    "osc_port": 9007,
    "recording_export_format": "fbx",   # "fbx", "bvh", or "npz"
...
class MotionGui(QtWidgets.QWidget):
    ...
    def _build_recording_mocap_dict(self):
        mocap_data = {
            ...
            "skeleton": self.synthesis.skeleton,  # Passing full dict prevents FBX 'root' KeyError
            "motion": {
    ...
    def _export_recording_npz(self, base_path):
        ...
        timestamps = (np.arange(frame_count, dtype=np.float32) / fps).astype(np.float32)

        np.savez(
            base_path + ".npz",
            **{
                "/mocap/0/joint/pos_local_timestamps": timestamps,
                "/mocap/0/joint/pos_local_values": pos_local.reshape(frame_count, -1),
                "/mocap/0/joint/rot_local_timestamps": timestamps,
                "/mocap/0/joint/rot_local_values": rot_local.reshape(frame_count, -1),
                "/mocap/0/joint/pos_world_timestamps": timestamps,
                "/mocap/0/joint/pos_world_values": pos_world.reshape(frame_count, -1),
                "/mocap/0/joint/rot_world_timestamps": timestamps,
                "/mocap/0/joint/rot_world_values": rot_world.reshape(frame_count, -1),
            }
        )
