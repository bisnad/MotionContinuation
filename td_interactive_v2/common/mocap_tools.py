...
class Mocap_Tools:
    ...
    def load_npz_topology(json_path):
        ...
        children = topo.get("children", build_children_from_parents(parents))
        joints = topo.get("joints", [f"joint_{j}" for j in range(len(parents))])
    ...
    def resample_npz_mocap_data(np_data, target_fps, time_ranges, parents, children, joints=None):
        ...
        rot_local = rot_np.normalize(fix_continuity(rot_local))
        rot_world = rot_np.normalize(fix_continuity(rot_world))
        ...
        for start_time, end_time in time_ranges:
            ...
            for j_idx in range(num_joints):
                ...
                if parent_idx == -1:
                    ...
                    offsets[j_idx] = rot_np.rot(quat_conjugate(parent_rot_world), bone_world.reshape(1, 3))[0]
