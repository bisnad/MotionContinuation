...
class MotionSynthesis():
    def __init__(self, config):
        ...
        self.cond_dim = config.get("cond_dim", 3)
        ...
        # Autoregressive memory for current condition tracking
        self.current_target_cond = torch.zeros((1, self.seq_length, self.cond_dim)).to(self.device)
    ...
    def _orthogonalize_pose(self, pose):
        # Defensive: if a full pose+condition vector arrives, discard condition channels
        if pose.shape[-1] == self.input_dim + self.cond_dim:
            pose = pose[..., :self.input_dim]
        ...
        expected_rot_dim = self.joint_count * 6
        if rot_6d.shape[-1] != expected_rot_dim:
            raise ValueError(
                f"_orthogonalize_pose: expected {expected_rot_dim} rotation dims, "
                f"got {rot_6d.shape[-1]} for pose shape {pose.shape}"
            )

        rot_6d = rot_6d.contiguous().view(*rot_6d.shape[:-1], self.joint_count, 6)
        rot_mat = rot_to.r6d_to_mat(rot_6d)

        x_axis = rot_mat[..., :, 0]
        y_axis = rot_mat[..., :, 1]
        rot_6d_norm = torch.cat([x_axis, y_axis], dim=-1)
        rot_6d_norm = rot_6d_norm.contiguous().view(*pose.shape[:-1], expected_rot_dim)
    ...
    def update(self):
        ...
        with torch.no_grad():
            full_input = torch.cat([self.motion_seq.unsqueeze(0), self.current_target_cond], dim=-1)

            log_pi, mu, sigma = self.model(full_input, return_sequence=False)

            pred_pose_cond_norm = self._sample_mdn(
                log_pi,
                mu,
                sigma,
                gaussian_temp=self.gaussian_temp,
                pi_temperature=self.pi_temperature,
                top_p=self.top_p
            )

            # Split model output into pose and target-condition channels
            raw_pred_pose = pred_pose_cond_norm[:, :, :self.input_dim]
            pred_cond = pred_pose_cond_norm[:, :, self.input_dim:]

            # Feed predicted condition autoregressively, same as training-time free running
            self.current_target_cond = pred_cond.expand(-1, self.seq_length, -1).detach().clone()

            # Filter ONLY the pose channels
            filtered_pose = self.one_euro_filter(raw_pred_pose)

            # Safety: ensure no condition channels are ever passed to pose orthogonalization
            filtered_pose = filtered_pose[:, :, :self.input_dim]

            self.pred_pose = self._orthogonalize_pose(filtered_pose)

            # Update autoregressive motion history with pose only
            self.motion_seq = torch.cat([self.motion_seq[1:, :], self.pred_pose.squeeze(0)], dim=0)
            ...
            rot_sequence_tensor = torch.tensor(rot_seq_6d, dtype=torch.float32, device=self.device)
            rot_matrices = rot_to.r6d_to_mat(rot_sequence_tensor)

            root_trajectory_tensor = torch.tensor(root_traj, dtype=torch.float32, device=self.device).unsqueeze(0)

            self.synth_pose_wpos, self.synth_pose_wrot_mat = self._forward_kinematics(
                rot_matrices,
                root_trajectory_tensor
            )
    ...
    def _sample_mdn(self, log_pi, mu, sigma, gaussian_temp=0.1, pi_temperature=1.5, top_p=0.9):
        ...
        pose_segment = sampled_val[:, :, :self.input_dim]
        cond_segment = sampled_val[:, :, self.input_dim:]

        ortho_pose = self._orthogonalize_pose(pose_segment)

        return torch.cat((ortho_pose, cond_segment), dim=-1)
